...
| **E14** | **`WITH SECURITY_ENFORCED` does not compile at v67 — measured, with the v66 control.** Prompted by a LinkedIn screenshot of the compile error, which is exactly the kind of source this project does not believe: a screenshot can be cropped, staged, or from another context, and it is someone else's org. So it was re-measured here in 2 minutes. Same source, only the apiVersion differs: **v66 deploys**, **v67 fails** — `WITH SECURITY_ENFORCED is no longer supported, use WITH USER_MODE instead. (3:27)`. The screenshot was right; that is not the point. **Implication for `_resolve()`: the `clause == "SECURITY_ENFORCED"` branch is checked BEFORE the apiVersion default, so it answers for a v67 class carrying that clause — a program that cannot exist.** Checked, and it is **not a bug**: the corpus has **no** SECURITY_ENFORCED case at all and `SecurityEnforcedAxesTest` uses `api=58.0`, so nothing asserts anything about the impossible combination; the branch is simply unreachable for org-sourced code. It would only surface analysing **undeployed source** in CI, where the honest report is "this will not compile", not a mode verdict. **Scope:** this measures **Apex at v67**. The Summer '26 SOQL and SOSL Reference still lists `WITH SECURITY_ENFORCED` as a valid `WITH` option, which may describe REST/Bulk/SOAP surfaces rather than Apex — **unmeasured, and do not assume the reconciliation.** |
| **E15** | **The record axis inside a trigger body — BOTH halves of the guide's sentence, measured in one experiment.** E13 left sharing and FLS in a v67 trigger explicitly unmeasured; a Stack Exchange thread then produced three positions that could not all be right (the guide: user mode *"effectively enforces a with sharing context in the trigger body"*; a commenter: v67 rectified edge cases so triggers are *"now always effectively **without** sharing"*; a third: measured bounded SOQL, **in their org**). The probe reports **two** numbers: `PLAIN` (no mode clause — reads the OPERATION's mode) and `SYS` (`WITH SYSTEM_MODE` — pins the operation, so the record axis falls to the trigger's **ambient context**, which is the only way to observe it at v67). Same trigger source, same host, same user, same admin-owned rows on a Private object; only the meta-xml number differs. **v58: `PLAIN=5 SYS=5`. v67: `PLAIN=0 SYS=5`.** So a plain read in a v67 trigger body **is bounded**, *and* the ambient context is **still `without sharing`** — it did not change, the operation's user-mode default overrides it. **Both commenters were right about different things**, and the guide's sentence is confirmed in both of its clauses at once. |
| **E16** | **A trigger body does NOT inherit its caller's sharing declaration.** Same probe, fired three ways — direct, from a `with sharing` class, from a `without sharing` class — at **both** versions. All six readings identical. So the edge-case claim does not reproduce **in the form an Apex caller can express**. **Do not widen this**: the claim was about *"certain automation contexts"*, and Flow, Process Builder, platform-event subscribers, Aura/LWC controllers and guest-user contexts are **not tested**. One form refuted, not the claim. |
...
blast_radius/
  ...
  test_*.py                 307 tests in 15 files
...
0. **FOUR v67 DOC CLAIMS THAT TOUCH THE PRECEDENCE LAW AND ARE UNMEASURED** — surfaced
   ...
   **experiments to run, never as edits to make** (§2's standing rule). Two of them
   ...
   - **(c) THE RECORD AXIS INSIDE A v67 TRIGGER BODY — ✅ MEASURED (E15), and it closed
     cleanly.** A plain read in a v67 trigger body **is bounded** (`PLAIN=0`), while the
     ambient context underneath is **still `without sharing`** (`SYS=5`). We were never
     at risk of the false positive this item worried about: `active_triggers()` carries
     only `{name, apiVersion, handler_min_api, dml_ops}` — no sharing field, no SOQL
     reads — and PS509 fires only below v67 on the CRUD axis, so we made no record-axis
     claim to overstate. **Two real gaps this measurement exposed, both still open:**
     - 🔴 **A v67 trigger with an EXPLICIT `WITH SYSTEM_MODE` in its body bypasses
       sharing** — `SYS=5` says so. PS509 fires only when `apiVersion < 67` and the
       trigger model carries **no mode clause**, so that trigger is invisible to us. A
       real escalation path, and the one this measurement found by accident.
     - A trigger's own **SOQL reads never enter the reach at all**. Understates rather
       than overstates — the safe direction — but it is a hole and PS509's wording
       should not imply otherwise.
   - **(d) FLS inside a v67 trigger.** E13's declared scope is object-CRUD only; E15
     closed the record axis but **not this one**. The doc-side evidence is thinner than
     it looks: the guide's worked example demonstrates **SOQL reads**, not DML. A Stack
     Exchange commenter reports measuring it (`Operation failed due to fields being
     inaccessible`) — **their org, not ours**. **Still unmeasured here.**
